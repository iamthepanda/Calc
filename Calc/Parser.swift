...
public indirect enum AST {
    ...
    static func fromString(_ str: String) throws -> AST {
        return try AST.fromString(str, withFunctions: [])
    }

    static func fromString(_ str: String, withFunctions functions: [String]) throws -> AST {
        let lexer = Lexer()
        let parser = Parser()
...
public class Parser {
    ...
    func parse(_ tokens: TokenStream) throws -> AST {
        var remainingTokens = tokens
        return try parseEquals(&remainingTokens)
    }

    //equalsexpr ::= <addexpr> [ = <equalsexpr> ]
    func parseEquals(_ tokens:inout TokenStream) throws -> AST {
    ...
    func parseAdd(_ tokens:inout TokenStream) throws -> AST {
        let leftExpr = try parseMul(&tokens)

        if (tokens.peek() ~== Token.TOpPlus) || (tokens.peek() ~== Token.TOpMinus)
        {
    ...
    func parseMul(_ tokens:inout TokenStream) throws -> AST {
        let leftExpr = try parsePow(&tokens)

        if (tokens.peek() ~== Token.TOpTimes) || (tokens.peek() ~== Token.TOpDivide)
        {
            let token = tokens.consume()!
    ...
    func parsePow(_ tokens:inout TokenStream) throws -> AST {
    ...
    func parseFunNumParen(_ tokens:inout TokenStream) throws -> AST {
    ...
    func parseNumParen(_ tokens:inout TokenStream) throws -> AST {
