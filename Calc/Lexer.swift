...
private extension String {
    func rangesOfString (str: String) -> [Range<Index>] {
        ...
        while let range = scratchString.range(of: str) {
            ranges.append(range)
            scratchString = scratchString.substring(from: range.upperBound)
        }
        return ranges
    }
}
...
func tokensFromLetters(_ str: String, strings: [String]) -> TokenStream {
    let sortedStrings = strings.sorted(by: {$0.characters.count > $1.characters.count})
    var locations : [Int:String] = [:]

    //scan for locations
    for tString in sortedStrings {
        let ranges = str.rangesOfString(str: tString)
        for range in ranges {
...
class Lexer {
    ...
    func scanStreamForStrings(_ tokens: TokenStream, strings: [String]) -> TokenStream {
        ...
        for token in tokens {
            ...
            case let t where scanString.characters.count > 0:
                ...
                resultStream.append(contentsOf: stringTokens)
                scanString = ""
    ...
    func tokenize(_ str: String) throws -> TokenStream {
        return try tokenize(str.characters.map{$0})
    }

    func tokenize(_ chars: [Character]) throws -> TokenStream {
    ...
    private func tokenizeNextToken(_ characters:inout [Character]) throws -> Token? {
    ...
    private func tokenizeNumber(_ characters:inout [Character]) -> Token? {
